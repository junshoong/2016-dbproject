...
class UserManager(BaseUserManager):
    def create_user(self, login_id, name, email, period, password=None, **kwargs):

        if not password:
            password = '0000'

        email = self.normalize_email(email),
        user = self.model(login_id, name, email, period, **kwargs)

        user.set_password(password)
        user.save(using=self._db)
        return user

    def create_superuser(self, login_id, name, email, period, password, **kwargs):
        user = self.create_user(login_id, name, email, period, password, position='admin', **kwargs)
